...
export class UserSoftDelete1761115475468 implements MigrationInterface {
  name = 'UserSoftDelete1761115475468';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'ventas_estado_enum') THEN
        CREATE TYPE "public"."ventas_estado_enum" AS ENUM('ABIERTA', 'PAGADA', 'ANULADA');
      END IF;
    END$$;`);
    ...
    await queryRunner.query(`DO $$
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_30abc4cd3cc8a4924e79226529d') THEN
        ALTER TABLE "compras_detalles" ADD CONSTRAINT "FK_30abc4cd3cc8a4924e79226529d" FOREIGN KEY ("compraId") REFERENCES "compras"("id") ON DELETE CASCADE ON UPDATE NO ACTION;
      END IF;
    END$$;`);
    await queryRunner.query(`DO $$
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_3c43196c87ab1d2e9796eeb8e38') THEN
        ALTER TABLE "compras_detalles" ADD CONSTRAINT "FK_3c43196c87ab1d2e9796eeb8e38" FOREIGN KEY ("productoId") REFERENCES "productos"("id") ON DELETE NO ACTION ON UPDATE NO ACTION;
      END IF;
    END$$;`);
    await queryRunner.query(`DO $$
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_354f0a5c4b4ba3e0c12c372b952') THEN
        ALTER TABLE "compras_detalles" ADD CONSTRAINT "FK_354f0a5c4b4ba3e0c12c372b952" FOREIGN KEY ("bodegaId") REFERENCES "bodegas"("id") ON DELETE NO ACTION ON UPDATE NO ACTION;
      END IF;
    END$$;`);
    await queryRunner.query(`DO $$
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_ccbc87d4d0703f780e5f55f1f04') THEN
        ALTER TABLE "compras" ADD CONSTRAINT "FK_ccbc87d4d0703f780e5f55f1f04" FOREIGN KEY ("proveedorId") REFERENCES "proveedores"("id") ON DELETE NO ACTION ON UPDATE NO ACTION;
      END IF;
    END$$;`);
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "ventas" ADD "formaPago" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "bodegas" ADD "estaActiva" boolean NOT NULL DEFAULT true`,
    );
    await queryRunner.query(
      `ALTER TABLE "productos" ADD "estaActivo" boolean NOT NULL DEFAULT true`,
    );
    await queryRunner.query(
      `ALTER TABLE "usuarios" ADD "estaActivo" boolean NOT NULL DEFAULT true`,
    );
    await queryRunner.query(
      `ALTER TABLE "clientes" ADD "estaActivo" boolean NOT NULL DEFAULT true`,
    );
